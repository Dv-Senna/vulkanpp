...
namespace vkpp
{
	...
	class PhysicalDevice
	{
		...
		private:
			int s_scoreGPU(VkPhysicalDevice device, const std::vector<const char *> &extensions);
			vkpp::QueueFamilyIndices s_getQueueFamiliesIndices(VkPhysicalDevice device);
			bool s_isValidGPU(VkPhysicalDevice device, const std::vector<const char *> &extensions);
